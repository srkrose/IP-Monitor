...
function ftpd_log() {
	cat /var/log/messages | grep -ie "$(if (($(date -d '1 hour ago' +"%-d") < 10)); then date -d '1 hour ago' +"%b  %-d %H:"; else date -d '1 hour ago' +"%b %d %H:"; fi)" | grep "pure-ftpd:" | grep "Authentication failed for user" | awk '{print $1,$2,$3,$6,$NF}' | sed 's/(?@//;s/)//;s/[][]//g' | awk '{printf "%-15s %-17s %-22s %-50s\n","DATE: "$1" "$2,"TIME: "$3,"IP: "$4,"USER: "$NF}' | sort | uniq -c >>$temp/ftpdlog_$time.txt
}
...
function static_ip() {
	if [ -r $temp/ftpdlog_$time.txt ] && [ -s $temp/ftpdlog_$time.txt ]; then
		...
		iplist=$(cat $temp/ftpdlog_$time.txt | awk '{print $8}' | sort | uniq)
...
function sort_log() {
	if [ -r $temp/failed-ftpd_$time.txt ] && [ -s $temp/failed-ftpd_$time.txt ]; then
		sortlog=$(cat $temp/failed-ftpd_$time.txt | awk '{for (i=0;i<NF;i++) {if($i=="ID:" && $(i+1)!="") print}}' | sort -k8)
...
function summary() {
	...
	if [ ! -z $ftpdlog ]; then
		...
		uniqipcount=$(cat $ftpdlog | awk '{print $8}' | sort | uniq | wc -l)
		...
		if [ ! -z $failedftpd ]; then
			...
			newuniqip=$(cat $failedftpd | awk '{print $8}' | sort | uniq | wc -l)
